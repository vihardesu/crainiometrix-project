...
export const CalendarContextProvider = ({ children }: PropsWithChildren) => {
    ...
    return (
        <AriaCalendarContext.Provider
            value={{
                value,
                onChange: (newValue) => setValue(Array.isArray(newValue) ? (newValue[0] ?? null) : newValue),
                focusedValue,
                onFocusChange: (newValue) => setFocusedValue(Array.isArray(newValue) ? newValue[0] : newValue),
            }}
        >
            {children}
        </AriaCalendarContext.Provider>
